...
#         background: linear-gradient(135deg, #288000, #49e600);
#     }
...
def login_signup():
    ...
    if st.sidebar.button("Play as Guest"):
        st.session_state.user = f"guest_{int(time.time())}"
        st.sidebar.success("Playing as Guest")
        return True
...
def get_user_chats(username):
    return load_users().get(username, {}).get("chats", [])
...
def process_documents(files):
    try:
        ...
        for file in files:
            if file.size > 200*1024*1024:
                st.error(f"{file.name} exceeds 200MB")
                continue
            suffix = pathlib.Path(file.name).suffix
            tmp = tempfile.NamedTemporaryFile(delete=False, suffix=suffix)
            tmp.write(file.getbuffer())
        ...
        st.error(f"Doc processing failed: {e}")
        return None
...
def main():
    ...
    st.markdown("""
    <style>
    .stApp { background: linear-gradient(135deg,#e60974,#000078); }
    .stChatMessage { border-radius:12px; padding:8px; margin-bottom:8px; }
    .stChatMessage[data-testid="stChatMessage-user"] { background: linear-gradient(135deg,#288000,#49e600); }
    .stChatMessage[data-testid="stChatMessage-assistant"] { background: linear-gradient(135deg,#89f7fe,#66a6ff); }
    .footer { position:fixed; bottom:0; width:100%; text-align:center; padding:5px 0; font-size:14px; background:white; }
    </style>
    """, unsafe_allow_html=True)
    ...
    chats = get_user_chats(st.session_state.user)
    if not chats:
        chats.append({"id": f"chat_{int(time.time())}", "title": "New Chat", "messages": []})
        save_user_chats(st.session_state.user, chats)
        st.session_state.current_chat_index = 0
        st.session_state.messages = []
    else:
        if st.session_state.current_chat_index is None or st.session_state.current_chat_index >= len(chats):
            st.session_state.current_chat_index = 0
        st.session_state.messages = chats[st.session_state.current_chat_index]["messages"]
    ...
    with st.sidebar:
        ...
        if models:
            names = [m.get("name") for m in models]
            st.session_state.model = st.selectbox("Chat Model", names, index=names.index(DEFAULT_MODEL) if DEFAULT_MODEL in names else 0)
            st.session_state.embed_model = st.selectbox("Embed Model", names, index=names.index(DEFAULT_EMBED_MODEL) if DEFAULT_EMBED_MODEL in names else 0)

        st.markdown("---")
        uploaded = st.file_uploader("Upload Docs", type=["pdf","txt","csv","docx"], accept_multiple_files=True)
        if uploaded:
    ...
    if prompt:
        ...
        if len(st.session_state.messages) == 1:
            chats[st.session_state.current_chat_index]["title"] = prompt[:40]
        ...
        with st.chat_message("assistant"):
            ...
            try:
                ...
                final_prompt = RAG_TEMPLATE.format(context=context, question=prompt) if context else prompt
                messages_copy = [{**m} for m in st.session_state.messages]
                ...
                if st.session_state.get("streaming", True):
                    ...
                    buffer = ""
                    decoder = getincrementaldecoder("utf-8")()
                    resp = chat_completion(messages_copy, stream=True)
                    for chunk in resp.iter_content(chunk_size=1024):
                        if not chunk: continue
                        text = decoder.decode(chunk)
                        buffer += text
                    ...
                    resp = chat_completion(messages_copy)
                    data = resp.json()
    ...
    st.markdown('<div class="footer">Developed by <strong>Abdul Hameed Rajput</strong></div>', unsafe_allow_html=True)
